...
function moneyFormat(int: number): string {
    let str = String(int)
    let x = str.split('.')
    let x1: string = x[0]
...
function $_GET(key: string) {
    let s = window.location.search.match(new RegExp(key + '=([^&=]+)'))
    return s ? s[1] : false
}

const btnOpenCartCounter: any = document.querySelector('.btnOpenCart .counter')
const submitCartFormEl: any = document.querySelector('#submitCartForm')
const deliveryTypesFormEl: any = document.getElementById('deliveryTypesForm')
const btnOpenGoodDetailsModalEls: any = document.querySelectorAll('.btnOpenGoodDetailsModal')

function generateModalCart(): void {
	...
	fetch('data.json')
		...
		.then( data => {
			...
			let html = '<tbody>'
			let cartData: any = JSON.parse(localStorage.getItem('cart'));
			let cartCount: any = Object.keys(cartData).length;
			...
			html += '</tbody>'
			const cartModalTableEl: any = document.querySelector('#cartModalTable')
			cartModalTableEl.innerHTML = html
			const totalSumEl: any = document.querySelector('#totalSum')
			totalSumEl.innerText = moneyFormat(totalSum)
			
			submitCartFormEl.setAttribute('data-totalsum', totalSum.toString())
			const btnOpenCartEl: any = document.querySelector('.btnOpenCart')
...
class Page {
	...
	public sitename: string = 'Гигантские раскраски ilo'
	public title: string
	...
	constructor(title: string, keywords, description) {
		this.title = title
...
window.onload = () => {
	...
	const formInputNameCity:HTMLFormElement | null = document.querySelector('form input[name=city]')
    if(formInputNameCity) {
        formInputNameCity.value = ymaps.geolocation.city;
    }
	const cartModalFormEl: HTMLFormElement | null = document.querySelector('#cartModal form')
    if(cartModalFormEl) {
	...
	fetch('data.json')
		...
		.then( data => {
			...
			for(var key in goods) {
				...
				if(goodUrl && goodUrl == goods[key].alias && goods[key].status === true) {
					let goodHtml = '<div class="ui stackable grid">'
						...
						goodHtml += '</div>'
					goodHtml += '</div>'
					const goodEl: any | null = document.querySelector('#good')
			...
			html += '</div>';
			const catalogEl: HTMLDivElement | null = document.querySelector('#catalog')
			if(catalogEl) {
						...
						fetch('data.json')
							...
							.then( data => {
								...
								if(good && good.status === true) {
									...
									html += '</div>';
									const goodDetailsModalBodyEl: any = document.querySelector('#goodDetailsModal .modal-body')
									goodDetailsModalBodyEl.innerHTML = html;
									let goodsSlider = tns({
	...
	const btnOpenCartEl: HTMLButtonElement | null = document.querySelector('.btnOpenCart');
    if(btnOpenCartEl) {
        btnOpenCartEl.onclick = (): void => {
            generateModalCart()
            $('#cartModal').modal('show')
        }
        btnOpenCartEl.oncontextmenu = () => false
	}
	...
	const filterLinks: any = document.querySelectorAll('.filter li a')
	if(filterLinks) {
...
if(document.body && document.querySelectorAll('.number-spinner-button')) {
	document.body.addEventListener('click', function (e) {
		if (e.target && e.target.classList.contains('number-spinner-button')) {
			let numberSpinnerInput: any = Array.from(e.target.parentNode.children).filter((child) => child !== e.target && child.tagName == 'INPUT');
...
const btnCloseGoodDetailsModalEl: HTMLButtonElement | null = document.querySelector('.btnCloseGoodDetailsModal')
if(btnCloseGoodDetailsModalEl) {
	btnCloseGoodDetailsModalEl.onclick = () => {
...
// LAZY LOAD
const lazyImages: any = document.querySelectorAll('.lazy');
...
let orderId: number | string = parseInt($_GET('order_id'))
if(orderId) {
	let promise = new Promise( (resolve, reject) => {
		...
		request.onload = function() {
			if (this.status >= 200 && this.status < 400) {
				...
				if(goodUrl != 'thanks') {
					const paymentOrderIdEl: any = document.getElementById('paymentOrderId')
					const paymentDescriptionEl: any = document.getElementById('paymentDescription')
					const paymentSumdEl: any = document.getElementById('paymentSum')
				...
				if( document.getElementById('paymentDetails') ) {
					...
					const paymentDetailsEl: any = document.getElementById('paymentDetails')
...
if(document.querySelector('#cartProcessForm')) {
	document.querySelector('#cartProcessForm').addEventListener('submit', function(e) {
		//e.preventDefault();
		let totalSum = parseFloat(submitCartFormEl.dataset.totalsum);
...
let scrollToTopBtn: any = document.querySelector('.scrollToTopBtn');
...
// LOAD
document.querySelectorAll('.include').forEach((tag: any) => {
...
let deliveryTypeEls: any = document.getElementsByName('delivery');
if(deliveryTypeEls) {
    for(let deliveryTypeEl of deliveryTypeEls) {
        deliveryTypeEl.onchange = function() {
			const addressFormInputEl: any= document.getElementById('addressFormInput')
            if( this.value != 'sd' ) {
...
if(document.getElementById('copyrightYear')) {
	...
	const copyrightYearEl: any = document.getElementById('copyrightYear');
	copyrightYearEl.style.color = 'black';
	copyrightYearEl.innerHTML = fullYear;
}
if(document.getElementById('cartFormSelect')) {
	let cartFormSelectEl: any = document.getElementById('cartFormSelect');
